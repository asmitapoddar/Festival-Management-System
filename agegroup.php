...
<nav class="navbar navbar-default" role="navigation">
	...
	<?php
	  ...
	  $res=mysqli_query($connection,"Select * from participants where Ageg='A'");
	  $res2=mysqli_query($connection,"Select * from participants where Ageg='B'");
	  $res3=mysqli_query($connection,"Select * from participants where Ageg='C'");
      if((!$res)&&(!res2)&&(!res3)){
	      die("Database query failed: " .mysql_error());
	  }
	  global $row2;
	  global $row3;
	  while(($row=mysqli_fetch_array($res))&&($row2=mysqli_fetch_array($res2))&&($row3=mysqli_fetch_array($res3))){
